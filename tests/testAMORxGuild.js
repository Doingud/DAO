...
describe("AMORxGuilds", function () {
  ...
  describe("AMOR deployment", function() {
    ...
    describe("Clone Target deployment", function() {
      it("Should deploy AMORxGuild", async function () {
        const [address1, address2, address3] = await ethers.getSigners();
  
        const cloneTarget = await ethers.getContractFactory("AmorGuildProxy");
  
        const clone = await cloneTarget.deploy();
    ...
    describe("Proxy Factory deployment", function() {
      it("Should deploy GuildFactory", async function () {
        ...
        const factory = await guildFactory.deploy(CLONE_TARGET.address, IMPLEMENTATION.address, AMOR_TOKEN.address);
    ...
    describe("AMORxGuild Withdrawals", function () {
      it("Should allow withdrawals of AMORxGUILD_ONE", async function () {
        const [address1, address2, address3] = await ethers.getSigners();
        const amountGuildOne = await GUILD_ONE.balanceOf(address1.address);

        expect(await GUILD_ONE.withdrawAmor(amountGuildOne)).
          to.emit(GUILD_ONE,"Transfer").
            withArgs(address1.address, ZERO_ADDRESS, amountGuildOne);
        
      });

      it("Should allow withdrawals of AMORxGUILD_TWO", async function () {
        const [address1, address2, address3] = await ethers.getSigners();
        const amountGuildTwo = await GUILD_TWO.balanceOf(address1.address);

        expect(await GUILD_TWO.withdrawAmor(amountGuildTwo)).
          to.emit(GUILD_TWO,"Transfer").
            withArgs(address1.address, ZERO_ADDRESS, amountGuildTwo);
        
      });

      it("Should allow withdrawals of AMORxGUILD_THREE", async function () {
        const [address1, address2, address3] = await ethers.getSigners();
        const amountGuildThree = await GUILD_THREE.balanceOf(address1.address);

        expect(await GUILD_THREE.withdrawAmor(amountGuildThree)).
          to.emit(GUILD_THREE,"Transfer").
            withArgs(address1.address, ZERO_ADDRESS, amountGuildThree);
